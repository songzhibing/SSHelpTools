...
//    }
    ...
    [UIView animateWithDuration:self.animateTime animations:^{
        ...
        self.floatView.frame  = CGRectMake(floatView.frame.origin.x, floatView.frame.origin.y, floatView.frame.size.width, mainBtn.frame.size.height);
        
    }completion:^(BOOL finished) {
        self.optionsList.frame = CGRectMake(self.optionsList.frame.origin.x, self.optionsList.frame.origin.y, weakSelf.frame.size.width, 0);
        
        // 变更menu图层
        self.floatView.frame = self.floatView.bounds;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SSHelpDropdownMenuItem *item = self.data[indexPath.row];
    
    CGFloat cHeight = self.optionItemHeight;//[self.dataSource dropdownMenu:self heightForOptionAtIndex:indexPath.row];
    
    UILabel * titleLabel = [cell viewWithTag:999];
    titleLabel.text  = item.title;//[self.dataSource dropdownMenu:self titleForOptionAtIndex:indexPath.row];;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelect) {
        self.didSelect(self, indexPath.row, self.data[indexPath.row]);
    }
    [self hideDropDown];
}
