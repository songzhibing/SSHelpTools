...
typedef struct AppleIPv4Header AppleIPv4Header;

__Check_Compile_Time(sizeof(AppleIPv4Header) == 20);
__Check_Compile_Time(offsetof(AppleIPv4Header, versionAndHeaderLength) == 0);
__Check_Compile_Time(offsetof(AppleIPv4Header, differentiatedServices) == 1);
__Check_Compile_Time(offsetof(AppleIPv4Header, totalLength) == 2);
__Check_Compile_Time(offsetof(AppleIPv4Header, identification) == 4);
__Check_Compile_Time(offsetof(AppleIPv4Header, flagsAndFragmentOffset) == 6);
__Check_Compile_Time(offsetof(AppleIPv4Header, timeToLive) == 8);
__Check_Compile_Time(offsetof(AppleIPv4Header, protocol) == 9);
__Check_Compile_Time(offsetof(AppleIPv4Header, headerChecksum) == 10);
__Check_Compile_Time(offsetof(AppleIPv4Header, sourceAddress) == 12);
__Check_Compile_Time(offsetof(AppleIPv4Header, destinationAddress) == 16);
...
static uint16_t apple_in_cksum(const void *buffer, size_t bufferLen) {
    // 
	size_t              bytesLeft;
    int32_t             sum;
...
@implementation SSHelpSimplePing
...
- (void)didFailWithError:(NSError *)error {
    id<SSHelpSimplePingDelegate>  strongDelegate;
...
- (NSData *)pingPacketWithType:(uint8_t)type payload:(NSData *)payload requiresChecksum:(BOOL)requiresChecksum {
    ...
    if (requiresChecksum) {
        ...
        icmpPtr->checksum = apple_in_cksum(packet.bytes, packet.length);
    }
...
- (void)sendPingWithData:(NSData *)data {
    ...
    switch (self.hostAddressFamily) {
        case AF_INET: {
            packet = [self pingPacketWithType:AppleICMPv4TypeEchoRequest payload:payload requiresChecksum:YES];
        } break;
        case AF_INET6: {
            packet = [self pingPacketWithType:AppleICMPv6TypeEchoRequest payload:payload requiresChecksum:NO];
        } break;
        default: {
            assert(NO);
        } break;
...
+ (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet {
    ...
    NSUInteger                  result;
    const struct AppleIPv4Header *   ipPtr;
    size_t                      ipHeaderLength;
    
    result = NSNotFound;
    if (packet.length >= (sizeof(AppleIPv4Header) + sizeof(AppleICMPHeader))) {
        ipPtr = (const AppleIPv4Header *) packet.bytes;
        if ( ((ipPtr->versionAndHeaderLength & 0xF0) == 0x40) &&            // IPv4
             ( ipPtr->protocol == IPPROTO_ICMP ) ) {
            ipHeaderLength = (ipPtr->versionAndHeaderLength & 0x0F) * sizeof(uint32_t);
            if (packet.length >= (ipHeaderLength + sizeof(AppleICMPHeader))) {
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (icmpHeaderOffset != NSNotFound) {
        icmpPtr = (struct AppleICMPHeader *) (((uint8_t *) packet.mutableBytes) + icmpHeaderOffset);

        receivedChecksum   = icmpPtr->checksum;
        icmpPtr->checksum  = 0;
        calculatedChecksum = apple_in_cksum(icmpPtr, packet.length - icmpHeaderOffset);
        icmpPtr->checksum  = receivedChecksum;
        
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == AppleICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    if (packet.length >= sizeof(*icmpPtr)) {
        ...
        if ( (icmpPtr->type == AppleICMPv6TypeEchoReply) && (icmpPtr->code == 0) ) {
            if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                uint16_t    sequenceNumber;
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    SSHelpSimplePing *    obj;
    
    obj = (__bridge SSHelpSimplePing *) info;
    assert([obj isKindOfClass:[SSHelpSimplePing class]]);
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != SSHelpSimplePingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != SSHelpSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
