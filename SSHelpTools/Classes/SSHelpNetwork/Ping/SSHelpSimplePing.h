...
@protocol SSHelpSimplePingDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, SSHelpSimplePingAddressStyle) {
    SSHelpSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    SSHelpSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    SSHelpSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface SSHelpSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<SSHelpSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) SSHelpSimplePingAddressStyle addressStyle;
...
@protocol SSHelpSimplePingDelegate <NSObject>
...
typedef struct AppleICMPHeader AppleICMPHeader;

__Check_Compile_Time(sizeof(AppleICMPHeader) == 8);
__Check_Compile_Time(offsetof(AppleICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(AppleICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(AppleICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(AppleICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(AppleICMPHeader, sequenceNumber) == 6);

enum {
    AppleICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    AppleICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    AppleICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    AppleICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
